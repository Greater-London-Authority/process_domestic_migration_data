...
create_gross_flows_from_pq <- function(od_flows_pq, rounding = 2) {
  ...
  inflow_query <- od_flows_pq %>%
    group_by(geography, year, gss_in, sex, age) %>%
  ...
  outflow_query <- od_flows_pq %>%
    group_by(geography, year, gss_out, sex, age) %>%
  ...
  inflows <- inflow_query %>%
    collect() %>%
    complete(nesting(geography, gss_code), age, year, sex, fill = list(internal_in = 0))

  outflows <- outflow_query %>%
    collect() %>%
    complete(nesting(geography, gss_code), age, year, sex, fill = list(internal_out = 0))

  gross_flows <- full_join(inflows, outflows, by = c("geography",
                                                     "gss_code",
                                                     "year",
                                                     "sex",
                                                     "age")
